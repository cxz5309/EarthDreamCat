...
public class NoteManager : MonoBehaviour
{
    ...
    float startingPoint;
    WaitForSeconds beatIntervalWait;
    void Awake()
    {
    ...
    public void InitNoteWithBPM(string stage)
    {
        ...
        //에네미 리스트를 먼저 만든 후에
        reachTime = beatInterval * 7f;//노트의 스피드(짧을수록 빠름)
        endTime = enemies[enemies.Count - 1].order * beatInterval;//종료 시간
        beatIntervalWait = new WaitForSeconds(beatInterval);
    ...
    public void MakeNote(Enemy enemy, int relativePosition)
    {
        GameObject obj = ObjectPoolContainer.Instance.Pop(IntToTypename(enemy.typeNum));//타입에 맞는 몬스터 오브젝트풀에서 생성
        obj.transform.position = spawnPoint[enemy.typeNum].position + new Vector3((relativePosition * 10)-(Mathf.Abs(relativePosition * 10)), 0);//위치 정해주기
